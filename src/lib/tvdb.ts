...
export async function getTvdbPosters(tvdbId: number, apiKey: string): Promise<TvdbArtwork[]> {
  const token = await getTvdbToken(apiKey)
  if (!token) return []

  try {
    const res = await fetch(`${TVDB_BASE}/series/${tvdbId}/extended?meta=artworks`, {
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: "application/json",
      },
    ...
    const json = await res.json()
    const artworks = json?.data?.artworks
    if (!Array.isArray(artworks)) return []

    const results: TvdbArtwork[] = []
    for (const art of artworks) {
      if (!art.image) continue
      const fullUrl = art.image.startsWith("http") ? art.image : `${ARTWORKS_BASE}${art.image.startsWith("/") ? "" : "/"}${art.image}`
      const artType = art.type === 2 ? "poster" : art.type === 7 ? "logo" : art.type === 3 ? "backdrop" : null
      if (!artType) continue

      results.push({
        file_path: fullUrl,
        url: fullUrl,
        vote_average: typeof art.score === "number" ? art.score : 0,
        aspect_ratio: artType === "poster" ? 0.667 : 2.0,
        iso_639_1: art.language && art.language !== "eng" ? art.language : null,
        source: "tvdb",
        is_textless: !art.language || art.language === "eng" || art.language === "",
        type: artType,
      })
    }
    return results
  } catch (e) {
    log.error("TVDB getTvdbPosters exception", { error: e instanceof Error ? e.message : String(e) })
    return []
  }
}
